...
#   ut = c()
#   for (i in 1:ur) {
#     ut[i] = length(unique(d$t[d$rep == i]))
#   }
#   times = matrix(NA, nrow = max(ut), ncol = ur)
#   for (i in 1:ur) {
#     for (j in 1:ut[i]) {
#       times[1:length(unique(d$t[d$rep == i])), i] = unique(d$t[d$rep ==i])
#     }
#   }
...
#   for (j in 1:ur) {
#     for (i in 1:max(ut)) {
#       MPNs[i, j] = getMPN(x = d$x[(d$rep %in% j) & (d$t ==times[i])], n = d$n[(d$rep %in% j) & (d$t ==times[i])], v = d$v[(d$rep %in% j) & (d$t ==times[i])])$Results$MPNCU.ml
...
#   mu0s = MPNs[1, ]
#   sum = 0 # change sum name, because it's a name function from the base package
#   lnLs = function(x, n, v, k, t, b, mu0) { # Create an new function in the package with that
#     for (m in 1:ur) {
#       x = d$x[(d$rep %in% m)]
#       n = d$n[(d$rep %in% m)]
#       v = d$v[(d$rep %in% m)]
...
#
#   if (plot == T) { # put that in a separate function
#     plotplot <- function(a, b, c) {
#       a
#       b
#       c
#     }
#     supergraph <- plotplot(par(mar = c(4, 4, 1, 1)), plot(times,logDiffs), abline(a = coef(Mast2)[2] * log10(exp(1)),b = -coef(Mast2)[1] * log10(exp(1)), lty = 1, col = "red"))
#     return(supergraph)
#   }
#   else {
#     kGot = cbind(confint(Mast2, parm = c("k"), method = "quad")[1],coef(Mast2)[1], confint(Mast2, parm = c("k"), method = "quad")[2])
#     colnames(kGot) = c("LP 2.5%", "MLE", "LP 97.5%")
#     rownames(kGot) = c("Decay Rate (k)")
